...
(def ^:private wait-timeout-ms 100)
...
(deftest connection-with-handlers
  (let [done (CountDownLatch. 3)
        ...
        conn (mqtt/connect! mosquitto.fixture/local-uri
                            {:on-connect-complete-fn (constantly (log-call :on-connect-complete-fn))
                             ; Cannot be tested unless we can simulate connection loss.
                             ;:on-connection-lost-fn (constantly (log-call :on-connection-lost))
                             :on-delivery-complete-fn (constantly (log-call :on-delivery-complete-fn))
                             :on-unhandled-message-fn (constantly (log-call :on-unhandled-message-fn))})]
    ...
    (is (= {:on-connect-complete-fn true
            :on-delivery-complete-fn true
            :on-unhandled-message-fn true}
           @calls-log))
...
(deftest basic-subscription
  (let [n 100
    ...
    (dotimes [_ n]
      (.publish conn "sage/test" (.getBytes "hello" "UTF-8") 1 false))
    (.await latch wait-timeout-ms TimeUnit/MILLISECONDS)
    (is (= 0 (.getCount latch)))
    (mqtt/disconnect-and-close! conn)))
...
(deftest different-subscriptions-with-different-handlers
  (let [n 50
    ...
    (dotimes [_ m]
      (.publish conn "sage/test/2" (.getBytes "hello" "UTF-8") 1 false))
    (.await latch1 wait-timeout-ms TimeUnit/MILLISECONDS)
    (.await latch2 wait-timeout-ms TimeUnit/MILLISECONDS)
